...
const ManageAllFoods = () => {
    ...
    useEffect(() => {
        setDataLoading(true);
        axios.get(`https://safe-ocean-22917.herokuapp.com/all-foods?currentPage=${currentPage}&&size=${size}`)
        .then((response) => {
    ...
    const handleFoodDelete = (e, id) => {             
        ...
        .then((value) => {
            if(value) {
                axios.delete(`https://safe-ocean-22917.herokuapp.com/delete-single-food?foodId=${id}`)
                .then((response) => {
